...
NUM_BULLETA = 40
BULLETA_SPEED = 2
BULLETA_RADIAN = math.pi*2/(NUM_BULLETA-1)
...
class Background:
    def __init__(self):
        ...
        for i in range(NUM_STARS):
            self.stars.append(
                (
                    pyxel.rndi(0, pyxel.width - 1), # a以上b以下のランダムな整数を返す
    ...
    def update(self):
        for i, (x, y, speed) in enumerate(self.stars): # enumerate：indexと要素
            y += speed
            if y >= pyxel.height:
                y -= pyxel.height
...
class Player:
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_A):
            for i in range(NUM_BULLETA):
                bulletA.append(BulletA(
                    self.x + (PLAYER_WIDTH - BULLET_WIDTH) / 2, self.y - BULLET_HEIGHT / 2, i*BULLETA_RADIAN
                ))
            pyxel.play(0, 0)
...
class BulletA(Bullet):
    ...
    def update(self):
        ...
        self.x += BULLETA_SPEED * math.cos(self.rad)
        self.y -= BULLETA_SPEED * math.sin(self.rad)
...
class Blast:
    ...
    def draw(self):
        pyxel.circ(self.x, self.y, self.radius, BLAST_COLOR_IN) # circ(x, y, r, color):半径r、色colの円を (x, y) に描画
        pyxel.circb(self.x, self.y, self.radius, BLAST_COLOR_OUT) # 円の輪郭線を描画


class App:
    def __init__(self):
        pyxel.init(120, 160, title="UFO Shooter")
        pyxel.load("shooter_ast.pyxres")
    ...
    def draw_title_scene(self):
        pyxel.text(35, 66, "UFO Shooter", pyxel.frame_count % 16)
